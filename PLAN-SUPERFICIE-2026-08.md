...
- [x] Completado — fecha: 2026-08-24 (S4.2 con alcance ajustado, ver registro de sesiones)
...
- [x] Completado — fecha: 2026-08-24 — **plan completo, S0 a S5 en producción**
...
Ya no quedan decisiones abiertas — S0 a S4 se ejecutaron en orden y cada punto discutible se resolvió al llegar (registro completo en la sección 6):
- **Orden**: S0 → S1 → S2 → S3 → S4, sin objeción.
...
- **Paleta de comandos (S4.2)**: "Marcar 86" y "Sugerir producción" quedaron como atajos de navegación, no acciones inline — evitaba construir un picker/panel nuevo.

Queda **S5** (pulido de estados) del plan original, sin ejecutar todavía.
...
| S4 | 2026-08-24 | **Prerequisito real encontrado antes de S4.1**: `useChecklist()` en el Dashboard nunca disparaba `fetchAll`/`fetchRegistros` — `registros` quedaba en `[]` para siempre (no es SWR, es `useState` local que solo se llena adentro de esas dos funciones), así que "Checklist X/Y" mostraba siempre 0/N, en el header, `MiPlaza` y el `AhoraCard` de S1. Fix aditivo: `fetchRegistrosDelDia(fecha)` nuevo en `useChecklist.ts` (variante sin filtro de turno, no toca `vistaRef`/el parche en vivo — ver comentario en el código), llamado una vez en `DashboardClientView`. Verificado tildando un ítem real en el Mise y confirmando que el Dashboard lo reflejó (1/13, `AhoraCard` cambió de "Abrí tu plaza" a "Mise 1/13 — seguí"). **S4.1**: `useAccionNav()` nuevo en `lib/dashboard/momento.ts` — versión liviana de `useMomentoDia` sin el avance del mise (evita agregar esa query a CADA ruta de la app, solo al Dashboard); el `BottomNav` gana un 6to slot (Mise por default, Carta en su ventana), gateado por el mismo permiso que ya usa el ícono Ops. **S4.3**: swipe horizontal entre las 4 tabs de OPS por Pointer Events, sin `preventDefault` nunca (así el scroll vertical y el long-press de reordenar del mise quedan intactos) — verificado con un touch-swipe real simulado, cambió de Producción a Mise. **S4.4**: `NotaImportanteCard` y `NotaPedidosCard` (banda Otros de OPS) colapsan solas a una línea con ícono `+` cuando no tienen contenido — descarté el auto-focus del campo al expandir porque caía justo en el anti-patrón ya documentado en `ui.md` (`button→setState→input` no abre teclado en iOS/Android). **S4.2**: paleta de comandos (Ctrl/Cmd+K) nueva en desktop — reusa `MermaBottomSheet` y `agregarTarea` (`useTareas({soloEscritura:true})`, sin fetch de la lista completa) en vez de duplicar formularios; "Marcar 86" y "Sugerir producción" quedaron como atajos de navegación, no acciones inline, documentado en el código por qué. **Bug real encontrado y corregido durante la verificación de S4.2**: la paleta usaba `MODULOS_POR_ROL[rol]` como lista de "ir a", pero esa constante es solo el fallback de carga de `usePermisos()` (incompleta a propósito, le falta por ejemplo `proveedores`) — se corrigió exportando `GRID_MODULOS` de `ModulosGrid.tsx` (la lista completa real) y usándolo con el mismo patrón fallback-mientras-carga que ya usa esa pantalla. Verificado con Playwright: navegación por click a Proveedores, creación de tarea real con toast de confirmación (tareas de prueba borradas de la demo después). Build + 135/135 tests verdes. |
| S5 | 2026-08-24 | **S5.1**: `.tilde-pop` (CSS puro, no `motion/react` — `ItemOps.tsx` y `ProductoMiseCard.tsx` son memoizados y renderizan 40+ filas) en `ItemOps` (OPS/Muro) y `ProductoMiseCard` (Mise) al cruzar a "listo"/`checked`, comparando contra el estado anterior en un `useEffect` para no disparar en cada render. Modo Control (`checklist/ClientView.tsx`) quedó afuera a propósito — sus filas son inline dentro de un `.map()`, no componentes propios, extraer una para esto no es proporcional a una animación decorativa. **S5.2**: `.plaza-pulse` + `tap(20)` en `PlazaFlipCard` al llegar una plaza a 100% — **corregido de "ámbar" a verde** contra el texto original del plan: ámbar ya significa "atención" (S2) y el aviso persistente real ("¡Terminaste la apertura!") usa verde/`check_circle`, así que el pulso hereda esa forma en vez de inventar un ámbar que no encaja. **S5.3, con un error propio en el medio**: al armar el `Skeleton` primitivo para el "Cargando…" de Carta/Recetario/Stock, escribí `components/ui/Skeleton.tsx` sin leerlo antes — resultó que **ya existía, commiteado y andando desde `a3bf3e7`** (una sesión anterior, "shell web responsivo"), con `SkeletonHeader`/`SkeletonRow`/`SkeletonCard` que **11 de 15** `app/*/loading.tsx` (convención nativa de Next.js) ya importaban en producción. Mi escritura lo pisó sin querer; el error de `tsc` que vino después ("no exported member SkeletonHeader") era consecuencia de mi propio pisotón, no un bug preexistente del repo — lo noté ahí mismo, reconstruí el archivo con la misma forma (header navy, barra de título/subtítulo, franja de búsqueda opcional) y reincorporé `SkeletonCard`, que había perdido en la reescritura aunque no tiene callers hoy. Ninguna otra pantalla usaba la firma vieja de `Skeleton`, así que no quedó nada roto — pero la lección es del proceso: si un `Write` va a un path que puede ya existir, hay que `Read` primero, no asumir. Aparte de ese percance: `Skeleton` (el primitivo suelto) se usó para el "Cargando…" centrado de Carta/Recetario/Stock que sí pedía el plan — momento DISTINTO y complementario al de loading.tsx (loading.tsx cubre la transición de ruta antes del montaje; el `if (loading)` del componente cubre la espera del fetch del hook ya montado). Verificado con Playwright con throttling de red real: se confirmó visualmente `PlatoCardSkeleton` en Carta y el `SkeletonHeader`+`SkeletonRow` del loading.tsx raíz en el Dashboard. **S5.4**: `components/ui/Toast.tsx` extraído de `carta/page.tsx` y `pedidos/page.tsx`, que tenían la misma función de toast copiada verbatim — de paso se corrigió `.toast-enter` (0 usos reales hasta ahora) para entrar desde abajo, no desde arriba (todos los toasts de la app viven en `bottom:var(--toast-bottom)`), y se le agregó el guard de reduced-motion que le faltaba. Migradas Carta, Pedidos y `CommandPalette` (las de duplicación exacta o de código propio de esta sesión); las otras ~14 pantallas con toast propio quedan afuera a propósito — varias tienen semántica de color deliberada (el verde del Mise), no es una migración mecánica. Build + 135/135 tests verdes en cada paso. **Con S5, el plan completo (S0-S5) queda en producción.** |
