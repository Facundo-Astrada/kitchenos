...
const FORM_EMPTY: FormProv = { nombre: '', rubro: '', telefono: '', dias_entrega: [] }

export default function ProveedoresPage({ embedded = false }: { embedded?: boolean }) {
  const router = useRouter()
  const RESTAURANTE_ID = useRestauranteId()
  const { proveedores, loading, error, agregarProveedor, actualizarProveedor, eliminarProveedor, fetchFacturas, guardarFactura, refetch: refetchProveedores } = useProveedores()
  ...
  function openEdit(p: Proveedor) {
    setEditProv(p)
    setForm({ nombre: p.nombre, rubro: p.rubro ?? '', telefono: p.telefono ?? '', dias_entrega: p.dias_entrega ?? [] })
    setFormError(null)
    setModalOpen(true)
  }
