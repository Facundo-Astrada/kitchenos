...
interface CoachPanelContentProps {
  // 'page' = pantalla dedicada /coach (mobile full-screen, header con padding de status bar).
  // 'dock' = columna fija de escritorio (DesktopShell) — mismo diseño, header compacto.
  variant?: 'page' | 'dock'
  // Solo la pantalla dedicada pisa kc_screen_context con {screen:'coach'} — el dock convive
  // con cualquier pantalla y NO debe pisar el contexto que esa pantalla ya escribió.
  writesScreenContext?: boolean
}
...
export function CoachPanelContent({ variant = 'page', writesScreenContext = false }: CoachPanelContentProps) {
  ...
  const headerPaddingTop = variant === 'dock' ? 14 : 46

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg)', position: 'relative' }}>
      {/* Header navy */}
      <div style={{ background: 'var(--navy)', padding: `${headerPaddingTop}px 16px 14px`, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      {historialOpen && (
        <div
          ...
          style={{ position: 'absolute', inset: 0, zIndex: 1100, background: 'rgba(0,0,0,.45)', display: 'flex', justifyContent: 'flex-end' }}
        >
          <div
          ...
          >
            <div style={{ background: 'var(--navy)', padding: `${headerPaddingTop}px 16px 14px`, display: 'flex', alignItems: 'center', gap: 10 }}>
