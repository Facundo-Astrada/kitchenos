...
import {
  PLAZAS_OPS, MODULOS_ASIGNABLES, nivelLabel, nivelColor,
  fieldStyle, labelStyle, btnPrimary, btnSecondary,
  type PuestoForm, EMPTY_PUESTO_FORM, objetivosDeForm,
} from './equipoShared'
import { PermisosPorRolPanel } from './PermisosPorRolPanel'
import { DescripcionPuestoWizard, type Tanda } from './DescripcionPuestoWizard'
import { FichaPuesto } from './FichaPuesto'
...
export function PuestosEditorPanel({
  ...
  const [wizardTanda, setWizardTanda] = useState<Tanda>(1)
    ...
    return (
      <div style={{ padding: 16 }}>
        ...
        <FichaPuesto
          puesto={p}
          modo="edicion"
          onEditarPuesto={startEditPuesto}
          onEditarTanda={t => { setWizardTanda(t); setWizardPuesto(p) }}
          onEliminar={miembrosDelPuesto.length === 0 ? async () => {
