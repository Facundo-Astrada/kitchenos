...
**P5 — Lint de diseño — cerrado (`ce05477`):** terminó siendo estático (Node, sin Playwright) en vez de lo escrito originalmente en §10 — `testing.md` ya advierte que Playwright es caro y no se agrega para lo que un chequeo más simple resuelve, y las dos verificaciones que sí necesitaban DOM real (targets, ámbar) se pudieron cubrir igual con grep estructurado sobre `<button>` + la técnica de hit-slop que esta misma sesión introdujo. Calibrado contra una corrida real, no contra la lista de deseos: el chequeo de hex/box-shadow quedó afuera (ruido masivo sobre deuda pre-existente), el de `confirm()` se scopeó a superficie de servicio (si no, mezclaba el bug real con debate de estilo de pantallas de gestión). `npm run lint:design`, documentado en `.claude/docs/testing.md`. Encontró 5 `confirm()` reales pre-existentes fuera de esta sesión (`produccion/page.tsx`, `salon/config/page.tsx`) — anotados en `PENDIENTES.md`, no arreglados acá (fuera del scope de "construir la herramienta"), no wireado a CI hasta que se resuelvan.

## Plan P0-P5 cerrado

Único pendiente de superficie: el **container-transform diferido de P2** (carta de plaza morfando al header del Mise — requiere fusionar los dos `return` de `ChecklistPage` en un árbol único con `AnimatePresence`, su propio bloque). Todo lo demás — constitución (`DESIGN.md`), investigación (P1, este documento), Mise vitrina (P2, 4 sub-bloques), relevo de turno (P3), Registro Servicio (P4), lint (P5) — shippeado, verificado (build+tsc+tests en cada commit) y en producción.
