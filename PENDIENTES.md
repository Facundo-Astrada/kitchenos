...
### La cuenta de Anthropic está sin crédito — toda la IA caída (25/08)
`GET /v1/messages` devuelve 400 `invalid_request_error: "Your credit balance is too low"`. No es un bug de código ni de los model IDs (verificados vigentes contra `GET /v1/models`): cae **todo** lo que usa IA — importar receta por foto/texto/voz, importar carta, escanear factura, importador universal, fichas técnicas PDF, listas de precios y el Kitchen Coach entero. Se reportó como "no se reconocen las fotos" porque los endpoints devolvían datos inventados en vez del error (ya arreglado, ver `PLAN-ACCESO-Y-USO-2026-08.md` B0.3). **Facundo lo carga él**; al hacerlo, verificar de paso que `ANTHROPIC_API_KEY` en Vercel sea la misma de `.env.local` (B0.2) y repetir la prueba contra la key de prod.
...
### Verificar en la app lo de `PLAN-ACCESO-Y-USO` (25/08)
Cinco de los siete bloques son UI y ninguno se probó corriendo la app — build, typecheck y 173 tests pasan, y las migraciones se verificaron contra la base real, pero eso no cubre la pantalla. En orden de riesgo:
- **Que Valentino entre** (`valentinocortesb@gmail.com`, Bros). Es el caso que originó todo: el vínculo `auth_user_id` está backfilleado y su puesto le da home, OPS, recetario, stock, pase y carta.
- **El toggle "Ve costos y food cost"** en el editor de puesto y el override por persona (cicla hereda/sí/no). Lo único de B3 que no se puede verificar sin la app.
- **La carta de bienvenida**: hace falta un usuario de prueba nuevo — solo aparece con `onboarding_visto_at` en null.
- **El modo pantalla completa de Producción en la tablet real.** Usa `zoom` (no `transform: scale`, que no reflowea y cortaría las columnas); si la tablet trae un navegador viejo, es lo primero a mirar.
- **El alta de receta con 3 etapas**, cargada de una pasada.
