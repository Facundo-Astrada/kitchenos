...
- Al "activar" un menú/evento en Producción (`activarMenu`, `app/(app)/produccion/page.tsx`) se crean `tareas` con plaza/receta/cantidad, pero **no** con `recipiente_nombre`/`peso_porcion`/`cantidad_ops` — esos campos finos, cargados en el `OpsPanel` del ítem al armar el menú en Carta, se guardan en `menu_preparaciones` pero nunca se muestran en Producción. **Ojo: NO es "hay que sincronizar a Mise"** — esa sync se probó y se revirtió a propósito (decisión del usuario, ver memoria `project_menus_unificacion`, "NO tocar Mise"). Es más chico: ¿vale la pena que `tareas`/`MenuActivoView` muestren esos datos (recipiente, peso) al ejecutar, o el editor está pidiendo más de lo que se usa después? Confirmar con Facundo antes de tocar.
...
- Jerarquía visual del ítem de composición: ya se separó Identidad/Producción y se fusionó Vincular+Nombre; falta una pasada más de diseño si el cliente lo sigue marcando en uso real.
