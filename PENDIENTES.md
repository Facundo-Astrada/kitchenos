...
"Copiar a otro día" e "Ingredientes consolidados" (se sacaron con la planilla legacy) — reimplementar sobre `tareas` si el usuario los pide. (Reset de `demanda_viva` al completar la apertura: resuelto 27/08, `useChecklist.resetDemandaViva`.)
...
**Clonar resuelto (27/08)**: select "Clonar permisos de otro puesto" en `PuestoFormBody` (Turnos → Puestos → nuevo/editar) copia `permisos_app` + `ver_costos` del puesto elegido, editable después de clonar. Sigue faltando la vista tipo matriz (puestos × módulos) para ver de un vistazo quién ve qué — no priorizada esta sesión (el botón de clonar es lo que sacaba más fricción).

### Capacitación — KDS/Muro sin mecanismo para mostrar su tour (encontrado 27/08 al cerrar la cobertura)
**Tours de `configuracion`, `coach` y `bitacora` resueltos (27/08)**: contenido en `lib/coach/tours.ts`, ruta→tour en `RUTA_A_TOUR`, y las tres pantallas ahora escriben `kc_screen_context` (les faltaba — sin eso el FAB nunca sabía qué tour mostrar aunque `useTourAutomatico` lo disparara). Confirmado funcionando contra la cuenta demo.

**`kds` y `muro` quedan con el contenido del tour escrito pero inalcanzable**: las dos viven bajo `app/(servicio)/layout.tsx`, que deliberadamente NO monta `KitchenCoachFAB` ni corre `useTourAutomatico` — es el layout de "Registro Servicio" (DESIGN.md §2: fondo fijo, cero animación de entrada, "se diseña quitando, no agregando"). Sumarles el FAB naranja de chat sería agregar justo lo que esa doctrina evita, así que no lo hice sin confirmar — es una decisión de diseño, no un bug. Alternativas a decidir con Facundo: (a) aceptar el FAB ahí igual, (b) un trigger mínimo propio (ícono "?" como el de Sidebar, sin todo el dock de chat), o (c) dejar esas dos pantallas sin tour a propósito (son tablet-first, casi siempre ya conocidas por el equipo antes de tocarlas). El contenido de los dos tours ya está escrito en `TOURS.kds`/`TOURS.muro`, listo para engancharse apenas se decida el mecanismo.
...
Los 5 ERROR de superficie de servicio (`produccion/page.tsx:326`, `salon/config/page.tsx` × 4) se resolvieron 27/08 reusando `ConfirmSheet`, ahora extraído a `components/ui/ConfirmSheet.tsx` (antes vivía duplicado por pantalla). El lint ya puede wirearse a CI sin quedar rojo por deuda ajena. Los ~80 restantes (Turnos, Clientes, HACCP, Carta, Stock, Espacios) son pantallas de gestión, no de servicio — el lint los marca WARN, no ERROR: ahí un confirm() nativo es debate de estilo, no el mismo bug (DESIGN.md §10 aplica a flujo de servicio).
...
- **`tareas` 594 kB** — lo más pesado, sin tocar (riesgo real de romper Planificación, ver abajo). La ventana de 60 días de `useTareas` hoy recorta 11 filas (preventiva, no ahorro real). Apretarla a ~3 semanas daría el salto, pero rompe Planificación al navegar a un día viejo: antes hay que hacer que consulte por su cuenta las fechas fuera de la ventana. No es un cambio chico — evaluar en sesión propia.
- ~~`productos` 66 kB~~ — **resuelto 27/08**: RPC `productos_criticos_count(restaurante_id)` (cuenta `stock_actual <= stock_critico` en el server, comparación entre columnas que PostgREST no soporta como filtro) reemplaza la descarga de hasta 1000 filas en `CoachPanelContent.useDatosClave`. Verificado devuelve el mismo número (100) que el cálculo cliente viejo.

### Ingeniería de menú siempre lee todo el historial de ventas, no el período de `/ventas` (investigado 27/08)
Causa confirmada — no son dos fuentes de datos distintas, es un filtro de fecha distinto: `/ventas` arranca en período **"mes"** (`useVentas().fetchVentas(desde, hasta)`, puede mostrar 0 si no hay ventas cargadas ESE mes), mientras Carta → Rentabilidad → Ingeniería usa `useVentas()` pelado, que trae la tabla `ventas` completa sin filtro de fecha (equivalente al período **"todo"** que `/ventas` sí ofrece como opción, solo que ahí no es el default). Fix chico aplicado: cartel en la pestaña Ingeniería aclarando que la popularidad es sobre todo el historial. Fix de fondo pendiente (decisión de producto, no bug): si Ingeniería debería scopear a un período razonable (¿mes? ¿90 días?) en vez de histórico completo — cambiaría qué platos caen en cada cuadrante, no lo hice sin confirmar con Facundo.

### Ocho funciones ya construidas que nadie encuentra (barrido 25/08)
La tabla completa está en `PLAN-ACCESO-Y-USO-2026-08.md` § B5.3, con criterio de priorización. El Muro ya dejó de estar huérfano: el modo pantalla completa de Producción tiene el primer link que existe en toda OPS. **Las 3 de mayor valor, resueltas 27/08**: Modo Control del mise (ícono `fact_check` ahora con etiqueta "Control" al lado, `checklist/ClientView.tsx`), escalar por ingrediente de referencia (la pista de doble tap se movió arriba de la lista de ingredientes en vez de al final, `recetario/[id]/page.tsx`), y "Sugerir producción" en OPS → Planificación (pasó de fondo translúcido a naranja sólido, mismo color que el resto de lo asistido por IA — competía en silencio contra "Cargar menú" y perdía). Quedan sin tocar, menor prioridad: paleta de comandos (Ctrl/K sin indicación, solo desktop), swipe entre tabs de OPS (sin affordance), guía del Mise (dos niveles de profundidad), vincular ingredientes con stock (botón sin explicar qué hace).
...
- ~~No hay botón de deshacer entrega~~ — **resuelto 27/08**: ícono "undo" en la barra persistente del cierre (`checklist/ClientView.tsx`), al lado de "Marcar salida", llama `useCierresTurno.deshacerEntrega()`.
- ~~Reportes → Auditoría sigue deduciendo~~ — **resuelto 27/08**: `fetchAuditoriaPaseTurno` ahora trae también `cierres_turno` del período y separa dos listas — "Pases entregados" (quién y a qué hora, hecho real) e "incumplidos" (ya no cuenta un grupo que tiene una entrega explícita, aunque el checklist haya quedado incompleto — ej. Modo Control).
