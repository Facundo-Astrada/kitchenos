...
### Refactor de Carta — días 7 y 9 del plan; el paso a paso completo en `refactor-kos.md` §2
**Día 6 resuelto (31/08):** `e2e/carta-smoke.spec.ts` (lista → toggle 86 → detail → Rentabilidad, 4 tabs) + borradas las ~200 líneas muertas (rama `view='nuevo'`+`handleCrear`, rama `isCreate` de FormView, `CAT_ICONS`) — `carta/page.tsx` bajó de 3.906 a 3.703 líneas, techo del ratchet en 3.710. Queda: moves puros (día 7: cards, exportadores, drawer, modal, form → sus propios archivos) y el panel OPS de `DetailView` ([carta/page.tsx:1435-1544](app/(app)/carta/page.tsx#L1435-L1544), día 9) que **duplica inline los helpers de `lib/ops/mise.ts`** con su 3ª copia de `PLAZAS_OPS` y sin `shrinkOrPruneMise` — mover una receta de plaza deja el `checklist_item` viejo sin achicar (bug latente de datos). Cada paso reversible y ≤1 día; el día 9 es el único con riesgo real (`/impacto` + matriz de verificación contra la base antes de tocarlo). Punto de parada explícito: el shell de ~700 líneas queda y está bien.
