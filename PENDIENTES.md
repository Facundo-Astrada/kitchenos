...
### `usePermisos` sin cache — repite hasta 4 queries en cada navegación
Auditoría 20/08: `usePermisos.ts` usa `useState`+`useEffect` plano (no SWR), hasta 4 queries a Supabase **en serie** (no `Promise.all`): `rol_permisos`, `restaurantes.configuracion`, y condicionalmente `equipo_miembros`→`puestos`. `RouteGuard` (`app/(app)/layout.tsx`) vive dentro del wrapper `motion.div` que se remonta en cada cambio de ruta, así que cada navegación repite las 4 queries desde cero y muestra pantalla en blanco hasta resolver (`RouteGuard.tsx`: `if (stillLoading) return null`). Encima 14 pantallas (Carta, Stock, Reportes, HACCP, Recetario, Pase, Configuración, Merma, Espacios, Tareas, Clientes, Turnos, Organigrama, Recetario/[id]) llaman `usePermisos()` por su cuenta además de `RouteGuard`, duplicando el trabajo — dos waterfalls simultáneos por navegación. Va contra la convención ya documentada en `hooks.md`#126 ("hooks keyed por restaurante_id usan SWR, no useState+useEffect"). Es el candidato principal a la queja de "tarda entre pantallas". Fix: migrar a SWR o a un context compartido tipo `AuthProvider` (que ya resuelve `restaurante_id` una sola vez y persiste). Relacionado, mismo hook: el bug de abajo (🟢) de que el catch traga el error real de esa query.
