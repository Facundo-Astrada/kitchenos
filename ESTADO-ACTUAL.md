...
| 25 | **Mesa de Trabajo** | `/espacios` | BETA (desktop) | `SegmentedTabs` con 2 pestañas: **Producción** (board anidado: Espacios físicos → Plazas (7 fijas) → Secciones (editables, tipadas: producción/almacén/heladera/freezer/estación) → sub-secciones (1 nivel) → Producciones/`checklist_items`, drag cross-plaza, panel OPS al clickear una producción) y **Stock** (nueva, jul 2026): board Kanban filas=productos/columnas=sectores físicos (`stock_sectores` → `stock_estantes`), drag&drop entre sectores/estantes con auto-scroll en los bordes y reorder manual dentro de la columna, columnas colapsables como chips (localStorage), multi-select Ctrl/Cmd+clic con mover en lote, buscador, eliminar producto del stock, editar nombre/ícono de sector (19 íconos de cocina), Stockear sigue el orden manual del board. Solo desktop (`useIsDesktop()`); mobile muestra mensaje amigable. |
| 24 | **OPS — Workspace diario** | `/operaciones` | Funcional | **Única puerta de entrada** al trabajo diario. **3 tabs**: Producción · Mise · Planificación (deep-link `?tab=`). Producción: modos Carta/Menú/Evento en **grid de columnas lado a lado** con sublabels (SP·Super Prioridad, P·Prioridad, REF·Refuerzo), QuickAdd con sugerencias de receta (≥3 chars), **columnas reordenables por drag&drop** (persistido en localStorage por restaurante+modo). **Modo "Todo"** (jul 2026, `ProduccionBoard.tsx`): reemplaza los recuadros planos por **bandas apilables por eje de trabajo** — banda Carta con una columna por **plaza** (foco: tocar la plaza la aísla a pantalla completa), bandas Menú/Evento con una columna por **paso** del menú, banda Otros (Pedidos + Limpieza + **Importante**, avisos con `@menciones` al equipo compartidos con Pase vía `pase_mensajes`). Checklist: auto-select plaza por rol, progreso por plaza en grid. **`/tareas`, `/checklist`, `/produccion` redirigen acá** (rutas viejas; la vista vive embebida) — **excepto perfil "emprendimiento"**, donde `/tareas` es una pantalla propia (ver fila 27). |
| 27 | **Modo Emprendimiento** | perfil por restaurante | Funcional (piloto: VOGLIO Farina) | `restaurantes.configuracion.perfil='emprendimiento'` oculta módulos de servicio de restaurante (Salón/KDS/Ops) incluso siendo admin (`MODULOS_EMPRENDIMIENTO` + `moduloEnPerfil()` en `usePermisos`). `/tareas` propia: lista agrupada por "área" libre en columnas Kanban + alta rápida + "Resumen del día" (pedidos/avisos/limpiezas). Mesa de Trabajo/Stock sin auto-seed de plazas de restaurante. |
...
**Resumen:** 27 módulos funcionales (salón y KDS completos, modo emprendimiento piloteado en VOGLIO Farina), 0 parciales, 0 críticos pendientes.
