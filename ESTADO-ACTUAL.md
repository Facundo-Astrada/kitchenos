...
| 24 | **OPS — Workspace diario** | `/operaciones` | Funcional | **Única puerta de entrada** al trabajo diario. **3 tabs**: Producción · Mise · Planificación (deep-link `?tab=`). Producción: secciones en **grid de columnas lado a lado** (jul 2026, ya no acordeón vertical) con sublabels (SP·Super Prioridad, P·Prioridad, REF·Refuerzo), toggle **Carta/Menú/Evento/Todo** (4° modo, ámbar: junta Carta+Menú+Evento en 5 recuadros — Carta/Menú/Evento + Pedidos + Limpieza), QuickAdd con sugerencias de receta (≥3 chars), **columnas reordenables por drag&drop** (persistido en localStorage por restaurante+modo). Recuadros fijos **Pedidos** (notas libres, no accionan nada) y **Limpieza** (HACCP de todas las plazas que tocan hoy, tildar = `registrarLimpieza()` real, alta rápida crea `haccp_limpieza`). Checklist: auto-select plaza por rol, progreso por plaza en grid. **`/tareas`, `/checklist`, `/produccion` redirigen acá** (rutas viejas; la vista vive embebida) — **excepto perfil "emprendimiento"**, donde `/tareas` es una pantalla propia (ver fila 27). |
