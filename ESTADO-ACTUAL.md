...
| 12 | **Reportes / CMV** | `/reportes` | Funcional | 12 tabs: Resumen, Ventas, CMV, Presupuesto, Rendimiento, Fuga, Food Cost, Compras, Precios, Producción, Caja, Auditoría. Presupuesto es mensual por 4 familias de gasto (materia prima 30%/personal 33%/alquiler 5%/gastos generales 17%) con real, desvío y EBITDA, botón "Usar estructura estándar". Fuga compara consumo teórico (ventas × ficha técnica) contra compras del período por producto, con tolerancia de merma esperada y lista aparte de platos sin receta vinculada (`lib/reportes/fuga.ts`). Export Excel contextual por tab. |
...
| 15 | **Producción / Planificación** | `/produccion` | Funcional (vive embebido en OPS) | Planilla del día, calendario mensual con multi-select (puntos verdes = menú, naranjas = evento), "Sugerir producción" con IA (motor de reglas + narración, nunca cambia números). Multi-menú por día separado visualmente: un bloque por menú activo (badge Menú/Evento, nombre, avance, vaciar solo ese) y filtro Todo/Menú/Evento con conteos, mismo criterio que el toggle de Producción. El picker de menú tiene dos acciones independientes por card: "Activar" (tareas del día/rango elegido) y **"Activar en el mise"** (checklist con apertura/cierre, mismo estado vigente/futuro/vencido que Carta → Menús, `estadoMiseMenu()` compartido). |
...
| 20 | **Kitchen Coach (IA)** | API `/api/coach` + FAB | Funcional | Chat con FAB draggable, tours guiados por pantalla (19/19 con cobertura), datos reales server-side (M1), tool use agéntico (crear tarea, marcar 86, registrar merma, sugerir producción — `crear_evento` todavía no setea plaza/prioridad/vigencia). Criterio de timing y tono (ago 2026): no sugiere cambios de receta/ideas durante servicio activo, y señala desvíos (food cost, faltantes) como dato a corregir, no como falla personal. Conoce menú/evento en el mise (vigencia, plaza de control, distinción con "Activar" de Planificación) y puede highlightear el botón Menús de Carta. |
...
| 5 | Info | Vitest: 92 tests en 5 archivos (máquina de estados, turnos/pase de turno, bus del mise, target/déficit, sincronización menú↔mise + plaza de control). Playwright: `e2e/salon-kds.spec.ts` (requiere `npx playwright install chromium` + dev server). Testing Library para hooks pendiente. | — |
