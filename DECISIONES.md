...
**Decisión:** K-OS no pasa a una estética de videojuego. Se construyó el prototipo completo (segunda tipografía, fondo oscuro, brasa, glow, partículas, racha, transiciones con overshoot) en `app/(app)/lab/page.tsx`, se miró contra la pantalla actual con un toggle **Calma / Arcade** sobre los mismos datos, y la respuesta fue **"de momento no lo implementamos"**. `DESIGN.md` queda **sin cambios**: sigue valiendo §4 (una sola familia tipográfica) y §10 (prohibido neón/glassmorphism, sombras y duraciones fuera de token). La ruta `/lab` se **borró del repo** una vez tomada la decisión: había salido a producción con el push y no tenía por qué quedar ahí una pantalla que no es producto, aunque solo se llegara por URL.
...
**Cómo se aplica:**
- **No abrir esto de nuevo sin una razón nueva.** Si vuelve el tema, se parte del prototipo y no de cero — pero el prototipo **ya no está en el árbol**: vive en el historial, en el commit `21be2b6`. Para traerlo de vuelta: `git checkout 21be2b6 -- "app/(app)/lab"` (o mirarlo sin tocar nada con `git show 21be2b6:"app/(app)/lab/page.tsx"`).
