...
| `menus` | `plaza_control TEXT NULL` (ago 2026) — si está cargada, `sincronizarMiseDeMenu()` manda TODAS las preparaciones a esa plaza al activar en el mise, pisando la `plaza` individual de cada una en `menu_preparaciones`. Pensada para una plaza custom sin cocina física (`usePlazasCustom`) que controla una sola persona, en vez de repartir el menú por estación. En el editor, `OpsPanel` recibe `forcedPlaza` y no deja elegir plaza por ítem | — |
