...
| `tareas` | `seccion` NOT NULL (default `'general'`). `modo` ('carta'=agrupa por PLAZA en `ProduccionBoard` / 'menu'y'evento'=por sección/paso). `menu_id UUID NULL` — tarea de menú activado, filtrada por `menu_id+turno_fecha`. `estado` (pendiente/en_curso/listo/duda). `asignado_a` TEXT — casi siempre `null` salvo `activarMenu` (hereda `usuario_asignado` del menú). `completado_por TEXT NULL` (ago 2026) — quién la pasó a `listo`, la escribe `useTareas.cambiarEstado()` con `perfil.miembro_id`, se limpia al despasar; misma convención que `checklist_registros.usuario_id`. **Carryover:** Producción muestra `turno_fecha=hoy` + ayer sin completar; **requiere `turno_fecha`** (NULL no se muestra). `plaza` agrupa el board de Carta/Todo (`ProduccionBoard`) — se manda `null` salvo altas que sí conocen la plaza (`handleAddItemBoard`, `handleCrearTareaDesdeItem`). `categoria='pedido_nota'` → notas libres del recuadro Pedidos, `turno_fecha=null` a propósito. `categoria='pase_turno'` → sheet de pase con día "Mañana", `turno_fecha`=mañana, sin `menu_id` a propósito | `completada` (no existe) |
