...
| `mesas` | `forma TEXT NOT NULL DEFAULT 'cuadrada'` ∈ `'cuadrada'\|'redonda'\|'rectangular'` + `ancho NUMERIC NOT NULL DEFAULT 8` + `alto NUMERIC NOT NULL DEFAULT 8` (ambos % del canvas, mismo sistema que `pos_x`/`pos_y` ya existentes) + `rotacion INT NOT NULL DEFAULT 0` ∈ `0\|45\|90` (jul 2026, Sesión 3 C3) — usados por el editor visual (`salon/config/page.tsx`) y el mapa real (`salon/page.tsx`, `MesaBoton`) para renderizar la mesa con su forma/tamaño/rotación reales en vez de un cuadrado fijo de 72px. | (agregado jul 2026) |
