...
| `lib/hooks/useMesas.test.ts` | `abrirCuenta`: reusa la cuenta abierta existente sin insertar, crea una nueva y marca la mesa ocupada, y atrapa el 23505 del candado `cuentas_mesa_abierta_unica` (dos aperturas concurrentes) para devolver la cuenta que ganó la carrera en vez de duplicar |
| `lib/hooks/useComandas.test.ts` | Bump online: el UPDATE de `comanda_items` no dispara un UPDATE de `comandas` desde el cliente (esa decisión es del trigger `trg_comanda_items_bump_actualiza_comanda`, no del hook) y `bumpearComanda` hace un solo `.in()`. Offline: el cálculo local optimista sigue funcionando (marca "lista" cuando corresponde, no adelanta si queda un ítem pendiente) y encola el bump para el reenvío |

Mock reusable en `lib/test-utils/mockSupabase.ts` — `createMockSupabaseClient()` arma un builder encadenable (`.from().select().eq()...`) que resuelve por nombre de tabla vía `setResponse(tabla, {data, error})`; no reimplementa filtros de PostgREST (eso ya lo cubren los tests de lógica pura de arriba). Si una tabla recibe una lectura Y una escritura con forma distinta en el mismo test (típico: `mutate()` revalida la lista después de un insert), setear por operación con `'tabla:metodo'` (ej. `'tareas:insert'`) — gana sobre la respuesta genérica de la tabla. `setResponse` también acepta un **array** para simular dos llamadas distintas a la misma key dentro de un mismo test (ej. un SELECT que no encuentra nada y, tras un error de carrera, un segundo SELECT que sí) — se consume en orden y queda clavado en el último valor al agotarse; una respuesta única sigue repitiéndose igual que siempre. Patrón de test completo: `vi.mock('@/lib/supabase/client', () => ({ createClient: vi.fn() }))`, mockear también `useRestauranteId`/`useAuth`/`useRestauranteConfig` (lo que el hook use) para aislarlo del resto del stack, `renderHook` con un wrapper `<SWRConfig value={{provider: () => new Map()}}>` (cache nueva por test, si no las keys quedan compartidas entre tests del mismo archivo). Requiere `// @vitest-environment jsdom` al principio del archivo — el resto de los tests corre en `node` (más rápido), jsdom es solo para los que rendereizan hooks de React.

**Vitest, ratchets** (`lib/ingenieria/ratchets.test.ts`, ago 2026) — no prueba lógica, audita el propio código fuente contra reglas que antes vivían solo en prosa: techos de líneas de las 5 pantallas grandes (`refactor-kos.md` §4, solo bajan), gotcha #20 de `hooks.md` (`createClient()` sin `useMemo` dentro del cuerpo directo de un hook — el chequeo distingue eso de un `createClient()` de un solo uso en un fetcher de módulo o en un `useCallback` anidado, que están bien), gotcha #18 (canal realtime sin `filter` por `restaurante_id`, con allowlist de tablas sin esa columna propia), la ley de `arquitectura-marco.md` ("use client" nunca en lógica de dominio, con allowlist de infraestructura de browser legítima) y `createAdminClient` sin `requireRestauranteId` en `app/api/**` (con allowlist de los que verifican sesión a mano de forma equivalente). Subir un techo o sumar un nombre a un allowlist es una decisión que se toma a mano en el archivo y se justifica en el commit.

**Playwright** — 2 specs, login real contra `admin@elrescoldo.com`, requieren `npx playwright install chromium` + `npm run dev` corriendo en `localhost:3000`: `e2e/salon-kds.spec.ts` (camino feliz salón → KDS → bump → reflejo en salón) y `e2e/carta-smoke.spec.ts` (lista → toggle 86 → detail → Rentabilidad, 4 tabs — la red del refactor de `carta/page.tsx` en curso, ver `refactor-kos.md` §2; corre en viewport mobile porque en desktop la lista usa `FlipCard` en vez de navegar directo a detail).
...
- **`Notification.permission` queda `'denied'` en Chromium headless sí o sí**, aunque el test llame `context.grantPermissions(['notifications'], {origin})` antes de navegar — límite de Chromium sin display, no del código ni de Playwright (confirmado: `serviceWorker`/`PushManager` sí están disponibles, solo la emulación de permisos de notificación no aplica). No perseguir el flujo real de `pushManager.subscribe()` en un spec headless — probar la rama "permiso denegado" de la UI (sí se puede) y pegarle directo a las rutas server-side (`page.request.post(...)`) con la sesión ya logueada para lo que hace falta verificar contra la DB real.
- **Exploración manual con Playwright contra `/operaciones`**: los 4 tabs (Producción/Mise/Planificación/Turno) están **todos montados a la vez** en una fila horizontal con scroll-snap (`.ops-swipe-track`), no con `display:none` — un locator por texto sin scopear (`page.locator('text=Parrilla')`) puede matchear el tab equivocado (ítems de Limpieza del tab Producción, no la plaza de Mise) porque Playwright los considera visibles igual. Scopear siempre con `.ops-swipe-track > div` `.nth(0|1|2|3)` (produccion/mise/planificacion/turno, en ese orden) antes de buscar texto adentro.
