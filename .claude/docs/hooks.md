...
- **Despachar una tarea desde el mise (`handleCrearTarea`, `checklist/ClientView.tsx`) debe mirar `checklist_item.menu_id`.** Si el ítem viene de un menú, la tarea va con `modo: 'menu'` (sin `plaza`) — nunca `modo: 'carta'`. Si no, cuando `plaza_control` fuerza `plaza='general'`, la tarea despachada aterriza en `ProduccionBoard` como columna "General" de la banda Carta, duplicando visualmente lo que `activarMenuParaFechas` ya puso en la banda Menú (mismo plato, mismo día, dos filas de `tareas`). Bug real encontrado ago 2026 en Bros.
