...
29. **`.upsert(data, { onConflict: 'a,b' })` falla con 42P10 si no existe un índice único exacto sobre esas columnas.** PostgREST traduce eso a `ON CONFLICT (a,b)` literal, sin `WHERE` ni expresión — una PK sobre `id`, un índice parcial o uno sobre `lower(a)` no sirven, Postgres no puede inferir el arbiter. Falla en TODAS las llamadas, no solo cuando hay un conflicto real, y si el catch no revisa `{ error }` (gotcha #2) el fallo queda invisible: la app reporta éxito y no escribe nada. Bug real (`/api/invitar`, sep 2026): la invitación se mandaba, el usuario se logueaba, pero `equipo_miembros` nunca se creaba — nadie lo notó porque ni ese insert ni el reparador posterior (`vincular`, que hace `UPDATE ... WHERE email=X AND auth_user_id IS NULL`, sin filas que matchear) tiraban error visible. Antes de usar `onConflict` sobre una tabla, verificar que el índice único existe tal cual (columnas exactas, mismo orden, sin `lower()` ni `WHERE`) — o crearlo en la misma migración.

30. **`npm run dev` (Next.js 16) rechaza levantar una segunda instancia del mismo proyecto aunque sea en otro puerto** — imprime "Ready" y después "⨯ Another next dev server is already running" con el PID de la primera, y sale con código 1. Si hace falta un dev server con env vars nuevas (ver #31) y ya hay uno corriendo (puede ser de otra sesión/terminal, no asumir que es propio), no alcanza con otro puerto: hay que matar el PID que loguea el error y arrancar de nuevo. Verificar con `netstat`/`Get-NetTCPConnection` antes de asumir que un puerto libre implica poder levantar otra instancia.

31. **`NEXT_PUBLIC_*` se inlinea al arrancar `next dev`/`next build`, no se relee en caliente.** Agregar una variable nueva a `.env.local` mientras un dev server ya está corriendo no la hace aparecer en el browser — sigue `undefined` hasta reiniciar ese proceso (mismo mecanismo que ya documentaba el gotcha de `NEXT_PUBLIC_SUPABASE_ANON_KEY` sobre `process.env[nombre]` dinámico). Si una pantalla "no muestra" algo gateado por una env var nueva, reiniciar el dev server antes de sospechar del código.
...
**Un endpoint nunca compara `user_restaurantes.rol` crudo — pasa por `mapRol()`** (`lib/permisos/roles.ts`): para toda la app `owner`/`compras` son admin y `sous_chef` es chef, y un `rol !== 'admin'` literal los deja afuera. Y la pantalla no ofrece lo que el endpoint va a rechazar: la regla (ej. `puedeInvitar`/`puedeAsignarNivel`) vive en `lib/permisos/` y la importan los dos lados. Caso real: Organigrama mostraba "Invitar" a todos y el server solo aceptaba admin — el chef llenaba el modal y chocaba con un toast de 3 segundos.
...
**`MODULOS_ASIGNABLES`** (`components/organigrama/equipoShared.ts` — mudado de `turnos/page.tsx` en sep 2026, cuando la ficha del equipo y el editor de puestos se mudaron a Organigrama) es la lista de módulos que el dueño puede tildar para un puesto/miembro desde la UI — tiene que cubrir todo `ModuloId` gateado en `SidebarNav.SECCIONES` o no hay forma de asignarlo (bug real, ago 2026: cubría 16 de 27). `'coach'` queda afuera a propósito: nunca está en `RUTA_A_MODULO`, el acceso es siempre libre. Ojo: `RUTA_A_MODULO` colapsa `/tareas`, `/checklist` y `/produccion` al permiso `'operaciones'` — restringir esos tres módulos individuales solo oculta el link del sidebar, la ruta directa sigue abierta si el puesto tiene `'operaciones'`.
...
**`RUTA_A_MODULO` acepta `string | string[]`.** Un valor array es OR: la ruta entra con cualquiera de esos permisos (`RouteGuard` y `useTourAutomatico` ya resuelven los dos casos). Se usa cuando varias pantallas viejas se consolidan en una sola con tabs (ej. Pedidos/Proveedores → tabs dentro de `/facturas`, sep 2026) y hay puestos reales con solo UNO de los permisos originales — gatear la ruta consolidada por un único permiso le cortaría el acceso a esa persona. Verificar con una query real sobre `puestos.permisos_app` si los permisos que se van a fusionar ya se otorgan siempre juntos antes de asumir que se puede colapsar a un solo id; si no, usar el array. La pantalla consolidada además filtra sus propios tabs por el permiso específico de cada uno (no todo el que entra ve todo) — ver `TAB_PERMISO` en `app/(app)/facturas/page.tsx`.

**Redirect con tab inicial: `URLSearchParams(window.location.search)`, no `useSearchParams()`.** El hook de Next exige un `<Suspense>` envolviendo toda la pantalla; el patrón ya establecido (`operaciones/page.tsx`, rutas viejas `/tareas`→`/operaciones?tab=X`) lee `window.location.search` a mano dentro de un `useEffect([])` que corre una sola vez al montar. **Bug real (sep 2026):** al copiar este patrón para `/pedidos`→`/facturas?tab=pedidos` se olvidó agregar el efecto lector en `facturas/page.tsx` — el redirect funcionaba pero siempre aterrizaba en el tab default, nunca en el que decía la URL. No se había notado antes en `/tareas` porque su tab (`'produccion'`) coincide por casualidad con el default de OPS. Al agregar un redirect con `?tab=`, verificar que el destino tenga el efecto lector, no asumirlo por el nombre del query param.
...
7. **`Date.now()` / `new Date()` dentro de un `useMemo` es una función impura en render** — el lint lo corta (`Cannot call impure function during render`) y tiene razón: devuelve algo distinto en cada re-render. Si el valor depende de "ahora" (días transcurridos, antigüedad, si venció), calculalo **en el fetcher** y devolvelo ya resuelto en el payload de SWR. Si además tiene que refrescarse solo, va en un `useEffect` con intervalo, nunca en el render.

8. **Un teaser en el Dashboard no reusa el hook completo de su pantalla dueña si ese hook auto-deriva datos de varias tablas.** `useCalendario()` fusiona `eventos` con eventos auto-generados desde `pedidos`/`produccion_diaria`/tareas de menú — pesado, y de más para una franja que solo necesita título+fecha de los próximos días (`useProximosDias`, sep 2026). Se escribió un hook angosto con selects propios sobre `eventos`/`menus` en vez de filtrar el resultado de `useCalendario()`.
