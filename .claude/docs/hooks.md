...
**Una sola puerta de activación por tipo de menú** (ago 2026, decisión de producto): un menú **fijo** se activa SOLO por vigencia en el mise — `ComposicionEditor`, `MenusView` y el picker de Planificación no ofrecen `activarMenuParaFechas` para fijo, y el editor ni siquiera muestra los campos de vigencia/plaza de control fuera de modo menú. Un **evento** se activa SOLO por fecha directa a Producción (`activarMenuParaFechas`) y nunca pasa por el mise. Antes las dos rutas convivían para cualquier tipo y podían duplicar tareas del mismo menú (una vez activada por fecha, otra vez despachada desde el mise).
...
- **Despachar una tarea desde el mise (`handleCrearTarea`, `checklist/ClientView.tsx`) debe mirar `checklist_item.menu_id`.** Si el ítem viene de un menú, la tarea va con `modo: 'menu'` (sin `plaza`, `seccion: menu_paso ?? 'general'`) — nunca `modo: 'carta'` ni un `seccion` hardcodeado. Antes de insertar, `handleCrearTarea` también chequea `tareas` (ya en memoria, sincronizado por realtime) por un duplicado vivo del mismo `checklist_item_id`+día+categoría y no reinserta si ya hay uno — dos cocineros despachando el mismo déficit a segundos de diferencia, o un doble-tap antes de que el `creating` local se propague, insertaban dos filas idénticas (bug real, Bros, fix ago 2026).
