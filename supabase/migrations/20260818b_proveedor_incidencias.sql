-- PLAN-4-CAPAS bloque B3 — proveedores: días de entrega e incidencias.
--
-- Parte A: horario_entrega. `dias_entrega` NO se toca — el plan original
-- pedía convertirla a INT[] ISO 1-7 (mismo criterio que checklist_rutina.dias_semana)
-- pero la columna ya existe en prod como text[] de etiquetas de día ("Lun"/"Lunes")
-- con datos reales cargados (Rescoldo, Bros). Migrarla es trabajo de limpieza de
-- datos fuera del alcance de esta sesión — se documenta el desvío acá y en
-- PLAN-4-CAPAS.md en vez de forzar la convención ISO sobre una columna que ya
-- cumple su función.
ALTER TABLE proveedores
  ADD COLUMN IF NOT EXISTS horario_entrega TEXT NULL;

-- Parte B: incidencias de proveedor. Se auto-crea una `faltante` al recibir
-- un pedido con menos cantidad de la pedida (usePedidos.recibirPedido); el
-- resto de los tipos se cargan a mano desde /proveedores.
CREATE TABLE IF NOT EXISTS proveedor_incidencias (
...
ALTER TABLE proveedor_incidencias ENABLE ROW LEVEL SECURITY;

CREATE POLICY "proveedor_incidencias_select" ON proveedor_incidencias FOR SELECT TO authenticated
  USING (restaurante_id = mi_restaurante_id());
CREATE POLICY "proveedor_incidencias_insert" ON proveedor_incidencias FOR INSERT TO authenticated
  WITH CHECK (restaurante_id = mi_restaurante_id());
CREATE POLICY "proveedor_incidencias_update" ON proveedor_incidencias FOR UPDATE TO authenticated
  USING (restaurante_id = mi_restaurante_id())
  WITH CHECK (restaurante_id = mi_restaurante_id());
CREATE POLICY "proveedor_incidencias_delete" ON proveedor_incidencias FOR DELETE TO authenticated
  USING (restaurante_id = mi_restaurante_id());

CREATE INDEX IF NOT EXISTS idx_proveedor_incidencias_restaurante ON proveedor_incidencias(restaurante_id);
CREATE INDEX IF NOT EXISTS idx_proveedor_incidencias_proveedor_fecha ON proveedor_incidencias(proveedor_id, fecha);

-- Reemplaza reset_demo_restaurante() completa (mismo patrón que las
-- migraciones reset_demo_* anteriores) para que horario_entrega y la tabla
-- proveedor_incidencias también se clonen al resetear la demo (R1).
