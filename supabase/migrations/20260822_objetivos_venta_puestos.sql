-- PLAN-4-CAPAS bloque B6 — objetivos de venta por puesto, con override por persona.
--
-- objetivos JSONB en puestos (base) y equipo_miembros (override puntual) — mismo
-- patrón que permisos_app/modulos_extra: el puesto define el objetivo, la
-- persona puede pisar una clave sin tocar el resto. Claves usadas hoy:
-- pct_comandas_con_postre, pct_comandas_con_cafe, ticket_promedio (lib/reportes/ventasPorPersona.ts).
ALTER TABLE puestos ADD COLUMN IF NOT EXISTS objetivos JSONB NOT NULL DEFAULT '{}'::jsonb;
ALTER TABLE equipo_miembros ADD COLUMN IF NOT EXISTS objetivos JSONB NOT NULL DEFAULT '{}'::jsonb;

-- Reemplaza reset_demo_restaurante() completa (mismo patrón que las migraciones
-- reset_demo_* anteriores) para que la columna objetivos de puestos también se
-- clone al resetear la demo (R1). equipo_miembros no se clona en esta función
-- (gap previo, fuera de alcance de este bloque).
