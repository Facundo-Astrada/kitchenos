-- PLAN-4-CAPAS bloque B7 — checklist de la carta pre-servicio.
--
-- "Una hora antes de abrir, el responsable de cocina irá partida por partida
-- con este checklist probando todas las elaboraciones" (elBulli 10.1). Contesta
-- ¿está bueno?, no ¿está hecho? (eso ya lo contesta el mise) — por eso es tabla
-- propia y no una sección de checklist_items/checklist_registros.
--
-- UNIQUE(carta_item_id, fecha, turno) → upsert con ese onConflict, mismo
-- patrón que checklist_registros. `turno` es el id de TurnoServicio
-- (almuerzo/cena), no fase apertura/cierre: el control se hace una vez por
-- servicio, no dos.
CREATE TABLE IF NOT EXISTS control_carta_registros (
...
ALTER TABLE control_carta_registros ENABLE ROW LEVEL SECURITY;

CREATE POLICY "control_carta_registros_select" ON control_carta_registros FOR SELECT TO authenticated
  USING (restaurante_id = mi_restaurante_id());
CREATE POLICY "control_carta_registros_insert" ON control_carta_registros FOR INSERT TO authenticated
  WITH CHECK (restaurante_id = mi_restaurante_id());
CREATE POLICY "control_carta_registros_update" ON control_carta_registros FOR UPDATE TO authenticated
  USING (restaurante_id = mi_restaurante_id())
  WITH CHECK (restaurante_id = mi_restaurante_id());
CREATE POLICY "control_carta_registros_delete" ON control_carta_registros FOR DELETE TO authenticated
  USING (restaurante_id = mi_restaurante_id());

CREATE INDEX IF NOT EXISTS idx_control_carta_registros_restaurante ON control_carta_registros(restaurante_id);

-- Reemplaza reset_demo_restaurante() completa (mismo patrón que las
-- migraciones reset_demo_* anteriores) para que control_carta_registros
-- también se clone al resetear la demo (R1).
