-- Suma bitacora_entradas/bitacora_items a reset_demo_restaurante() —
-- sin esto la demo queda sin Bitácora tras el reset nocturno.
-- Reemplaza la función completa (mismo patrón que las migraciones
-- reset_demo_* anteriores) agregando la fase de borrado (hija -> padre)
-- y la fase de clonado (padre -> hija) para las dos tablas nuevas.
...
;
  ...
  INSERT INTO productos (id, nombre, unidad, stock_actual, stock_minimo, stock_critico, categoria, proveedor_id, restaurante_id, activo, created_at, updated_at, precio_unitario, categoria_id, unidad_compra, cantidad_por_envase, unidad_uso, es_produccion, receta_id, sector_id, fuera_de_uso, estante_id, orden_sector)
  SELECT m.new_id, t.nombre, t.unidad, t.stock_actual, t.stock_minimo, t.stock_critico, t.categoria, f_proveedor_id.new_id, v_dest, t.activo, t.created_at, t.updated_at, t.precio_unitario, f_categoria_id.new_id, t.unidad_compra, t.cantidad_por_envase, t.unidad_uso, t.es_produccion, f_receta_id.new_id, f_sector_id.new_id, t.fuera_de_uso, f_estante_id.new_id, t.orden_sector
  FROM productos t
