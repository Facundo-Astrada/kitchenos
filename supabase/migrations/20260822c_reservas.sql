-- PLAN-4-CAPAS bloque B8 — Reservas: el modelo y la pantalla.
--
-- Tabla aislada (sin enganches todavía — eso es B9). mesa_id opcional
-- (se asigna recién al sentar, ver B9); nombre+telefono redundantes con
-- clientes a propósito, la mayoría de las reservas no tiene cliente
-- cargado como entidad. no_show es estado, no borrado (estadística de
-- cumplimiento para más adelante).
...
CREATE INDEX IF NOT EXISTS idx_reservas_restaurante_fecha ON reservas (restaurante_id, fecha);

ALTER TABLE reservas ENABLE ROW LEVEL SECURITY;

CREATE POLICY "reservas_select" ON reservas FOR SELECT TO authenticated
  USING (restaurante_id = mi_restaurante_id());
CREATE POLICY "reservas_insert" ON reservas FOR INSERT TO authenticated
  WITH CHECK (restaurante_id = mi_restaurante_id());
CREATE POLICY "reservas_update" ON reservas FOR UPDATE TO authenticated
  USING (restaurante_id = mi_restaurante_id())
  WITH CHECK (restaurante_id = mi_restaurante_id());
CREATE POLICY "reservas_delete" ON reservas FOR DELETE TO authenticated
  USING (restaurante_id = mi_restaurante_id());

-- Reemplaza reset_demo_restaurante() completa (mismo patrón que las
-- migraciones reset_demo_* anteriores) para que reservas también se clone
-- al resetear la demo (R1). cliente_id siempre se remapea a NULL: clientes
-- no está en el clon de la demo (gap previo a este plan, ver columnas.md).
...
BEGIN
  ...
  DELETE FROM control_carta_registros WHERE restaurante_id = v_dest;
  ...
  DELETE FROM reservas WHERE restaurante_id = v_dest;
...
;
  ...
  -- reservas (PLAN-4-CAPAS B8) — cliente_id siempre NULL: clientes no se
  -- clona a la demo (mismo criterio que mozo_id/usuario_id abajo).
  CREATE TEMP TABLE map_reservas(old_id uuid PRIMARY KEY, new_id uuid) ON COMMIT DROP;
  INSERT INTO map_reservas SELECT id, gen_random_uuid() FROM reservas WHERE restaurante_id = v_src;
  INSERT INTO reservas (id, restaurante_id, fecha, hora, pax, cliente_id, nombre, telefono, mesa_id, estado, origen, nota, creado_por, created_at)
  SELECT m.new_id, v_dest, t.fecha, t.hora, t.pax, NULL, t.nombre, t.telefono, f_mesa_id.new_id, t.estado, t.origen, t.nota, t.creado_por, t.created_at
  FROM reservas t
  JOIN map_reservas m ON m.old_id = t.id
...
;
  ...
  INSERT INTO puestos (id, nombre, descripcion, tareas_funciones, permisos_app, restaurante_id, created_at, nivel, plaza_default, objetivos)
  SELECT m.new_id, t.nombre, t.descripcion, t.tareas_funciones, t.permisos_app, v_dest, t.created_at, t.nivel, t.plaza_default, t.objetivos
  FROM puestos t
...
;
  ...
  -- control_carta_registros (PLAN-4-CAPAS B7)
  CREATE TEMP TABLE map_control_carta_registros(old_id uuid PRIMARY KEY, new_id uuid) ON COMMIT DROP;
  INSERT INTO map_control_carta_registros SELECT id, gen_random_uuid() FROM control_carta_registros WHERE restaurante_id = v_src;
  INSERT INTO control_carta_registros (id, restaurante_id, carta_item_id, fecha, turno, estado, nota, usuario_id, created_at)
  SELECT m.new_id, v_dest, f_carta_item_id.new_id, t.fecha, t.turno, t.estado, t.nota, t.usuario_id, t.created_at
  FROM control_carta_registros t
  JOIN map_control_carta_registros m ON m.old_id = t.id
