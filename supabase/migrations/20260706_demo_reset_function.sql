-- ================================================================
-- Migracion: reset_demo_restaurante() - clonado nocturno de la demo publica
-- Fecha: 2026-07-06
--
-- Borra TODO lo que exista hoy en el restaurante demo
-- (00000000-0000-0000-0000-000000000002) y lo vuelve a poblar con una copia
-- fresca de El Rescoldo real (00000000-0000-0000-0000-000000000001),
-- regenerando ids (uuid nuevos) y remapeando todas las FK internas via
-- tablas temporales old_id -> new_id (ON COMMIT DROP).
--
-- NO toca: restaurantes, user_restaurantes, equipo_miembros, rol_permisos
-- (ya resueltos manualmente para la demo) ni demo_visitas (analytics).
-- NO clona: perfiles (0 filas, PK = id de auth.users, tabla no usada por
-- la app - solo scripts/diagnose.mjs) ni turnos (17 filas en origen,
-- pero turnos.miembro_id es NOT NULL + FK real a equipo_miembros, que no
-- se clona; ademas UNIQUE(miembro_id,fecha) no scoped por restaurante ->
-- forzar un solo miembro_id de destino colisionaria en esa unique).
--
-- Referencias a equipo_miembros que SI se clonan como columna (mozo_id en
-- comandas/cuentas) se fuerzan a NULL porque el equipo del clon es
-- independiente del equipo de origen.
--
-- Atomico por construccion: toda la funcion corre dentro de la misma
-- transaccion que la sentencia que la invoca (una funcion plpgsql es
-- siempre transaccional); si cualquier INSERT/DELETE falla, Postgres
-- hace rollback de todo (no se puede dejar el destino a medio borrar).
--
-- Constraints UNIQUE no scopeadas por restaurante_id: se revisaron TODAS
-- las UNIQUE del esquema (information_schema.table_constraints +
-- pg_indexes) - la unica global es restaurantes.slug, que no aplica
-- porque esta tabla no se toca. El resto de las UNIQUE (config_fiscal,
-- fiscal_config, fiscal_tickets, presupuestos, categorias_producto,
-- checklist_registros, checklist_rutina_registros, espacio_plazas,
-- produccion_diaria, turnos) estan scopeadas por restaurante_id (directo
-- o via FK a una fila ya scopeada) -> no requieren tratamiento especial.
-- ================================================================
...
;
  ...
  INSERT INTO presupuestos (id, restaurante_id, periodo, monto, created_at, updated_at)
  SELECT m.new_id, v_dest, t.periodo, t.monto, t.created_at, t.updated_at
  FROM presupuestos t
...
;
  ...
  INSERT INTO proveedores (id, nombre, rubro, telefono, dias_entrega, restaurante_id, activo, created_at, updated_at)
  SELECT m.new_id, t.nombre, t.rubro, t.telefono, t.dias_entrega, v_dest, t.activo, t.created_at, t.updated_at
  FROM proveedores t
...
;
  ...
  INSERT INTO productos (id, nombre, unidad, stock_actual, stock_minimo, stock_critico, categoria, proveedor_id, restaurante_id, activo, created_at, updated_at, precio_unitario, categoria_id, unidad_compra, cantidad_por_envase, unidad_uso, es_produccion, receta_id)
  SELECT m.new_id, t.nombre, t.unidad, t.stock_actual, t.stock_minimo, t.stock_critico, t.categoria, f_proveedor_id.new_id, v_dest, t.activo, t.created_at, t.updated_at, t.precio_unitario, f_categoria_id.new_id, t.unidad_compra, t.cantidad_por_envase, t.unidad_uso, t.es_produccion, f_receta_id.new_id
  FROM productos t
