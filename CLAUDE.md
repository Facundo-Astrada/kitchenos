...
| **Monitoreo** | Salud de producción | `/api/cron/health` (corre solo a diario y devuelve **503** cuando algo está roto, así la corrida fallida se ve en Vercel — claves, handshake del realtime, base; pegarle a mano con `curl -H "Authorization: Bearer $CRON_SECRET"`), `/prod-status` (advisories de Supabase, estado del último deploy, deuda técnica abierta), skill `vercel-optimize` (costo/performance de lo ya deployado) |
